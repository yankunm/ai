...
with open("Mail Merge Project Start/Input/Names/invited_names.txt") as names_file:
    names = names_file.readlines() # read lines as a list
    
    # remove all the \n from each name
    for name in names:
        name = name.strip() # removes any leading or trailing whitespace characters
        new_letter = letter_contents.replace("[name]", name) # replace placeholder with actual name
